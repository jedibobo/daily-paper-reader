...
tldr: 针对大规模知识图谱（KG）上图神经网络（GNN）推理效率低、冗余计算多的问题，本文提出KG-WISE系统。该系统通过将GNN模型分解为细粒度组件，并利用大语言模型（LLM）生成查询模板以提取语义相关的子图，实现了查询感知的紧凑模型实例化。实验表明，KG-WISE在保持精度的同时，显著提升了推理速度并大幅降低了内存占用，为大规模图推理提供了高效的自适应方案。
selection_source: fresh_fetch
motivation: 现有的GNN加速方法通常采用静态模型且需全量加载，无法根据具体查询的结构和语义动态优化，导致在大规模KG上存在严重的数据冗余和计算浪费。
method: 提出KG-WISE范式，通过将GNN模型分解为细粒度可加载组件，并利用LLM生成可重用的查询模板来提取语义相关的子图，实现查询感知的模型实例化。
result: "在包含多达4200万节点的大规模KG上，KG-WISE实现了最高28倍的推理加速和98%的内存节省，同时保持或提升了预测精度。"
conclusion: KG-WISE证明了结合LLM的语义引导与细粒度模型分解能有效解决大规模知识图谱上的GNN推理瓶颈，实现了高效且精准的查询响应。
---

## 摘要
大规模知识图谱（KG）上图神经网络（GNN）的高效推理对于许多现实应用至关重要。GNN 推理查询的计算开销巨大且复杂度各异，因为每个查询涉及不同数量的目标节点，这些节点连接到具有不同密度和结构的子图。现有的加速方法（如剪枝、量化和知识蒸馏）虽然实例化了更小的模型，但未能根据单个查询的结构或语义进行调整。它们还将模型存储为必须完整加载的单体文件，错失了仅检索与目标节点语义相关的邻居节点及相应模型组件的机会。这些局限性导致在大规模知识图谱上出现过量的数据加载和冗余计算。本文提出了 KG-WISE，一种面向大规模知识图谱的任务驱动推理范式。KG-WISE 将训练好的 GNN 模型分解为细粒度组件，这些组件可以根据查询子图的结构进行部分加载。它利用大语言模型（LLM）生成可重用的查询模板，为每个任务提取语义相关的子图，从而实现查询感知且紧凑的模型实例化。我们在六个拥有多达 4200 万个节点和 1.66 亿条边的大规模知识图谱上评估了 KG-WISE。与最先进的系统相比，KG-WISE 在保持或提高准确性的同时，推理速度提升了高达 28 倍，内存占用降低了 98%，且在商业和开源权重 LLM 上均表现出色。
