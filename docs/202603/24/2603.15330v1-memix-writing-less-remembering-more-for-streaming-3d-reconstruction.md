...
title_zh: MeMix：在流式三维重建中减少写入，增加记忆
...
## 摘要
重建是三维视觉中的一项基础任务，也是空间智能的一项基本能力。特别是，流式三维重建对于实时空间感知至关重要，然而现有的循环在线模型在长序列上往往会因为状态漂移和遗忘而遭受渐进式性能退化，这促使了推理阶段补救措施的出现。我们提出了 MeMix，这是一个无需训练、即插即用的模块，它通过将循环状态重构为记忆混合体（Memory Mixture）来改进流式重建。MeMix 将状态划分为多个独立的记忆补丁（memory patches），并仅更新对齐程度最低的记忆补丁，同时精确保留其他补丁。这种选择性更新在保持 $O(1)$ 推理内存的同时缓解了灾难性遗忘，且无需微调或额外的可学习参数，使其能够直接应用于现有的循环重建模型。在标准基准测试（ScanNet、7-Scenes、KITTI 等）中，在相同的骨干网络和推理设置下，MeMix 在 7-Scenes 的 300-500 帧流上平均降低了 15.3% 的重建完整性误差（最高可达 40.0%）。代码可在 https://dongjiacheng06.github.io/MeMix/ 获取。
