...
## 摘要
大语言模型 (LLMs) 和智能体系统在自动化软件开发方面展现出巨大潜力，但由于软件逻辑与物理硬件行为之间的紧密耦合，将其应用于硬件在环 (HIL) 嵌入式和物联网 (IoT) 系统仍面临挑战。由于时序约束、外设初始化要求或特定硬件行为，成功编译的代码在部署到真实设备时仍可能失败。为了应对这一挑战，我们引入了一种用于 HIL 嵌入式开发的基于技能的智能体框架，以及 IoT-SkillsBench，这是一个旨在系统评估真实嵌入式编程环境下 AI 智能体的基准测试。IoT-SkillsBench 涵盖了三个具有代表性的嵌入式平台、23 个外设以及三个难度级别的 42 个任务，每个任务在三种智能体配置（无技能、LLM 生成技能和人类专家技能）下进行评估，并通过真实硬件执行进行验证。通过 378 次硬件验证实验，我们表明，包含结构化专家知识的简洁人类专家技能可以在不同平台上实现近乎完美的成功率。
