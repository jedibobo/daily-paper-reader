...
tldr: "针对时序图神经网络（TGN）推理效率低下的问题，本文提出了首个GPU高效的流式推理系统StreamTGN。传统方法在处理新边时会触发全局节点更新，而StreamTGN利用时序图更新的局部性原理，通过脏标记传播仅更新受影响的L跳邻居节点，将复杂度从O(|V|)降至O(|A|)。结合自适应重建调度和批处理流技术，该系统在保证零精度损失的前提下，实现了最高达739倍的加速，显著提升了动态图任务的实时处理能力。"
selection_source: fresh_fetch
motivation: 现有TGN推理系统在处理新边时会触发全局节点更新，导致计算冗余且难以满足实时流式处理的需求。
method: 利用时序图更新的局部性，通过脏标记传播识别受影响节点集，并采用GPU驻留内存和自适应重建调度优化推理。
result: 在八个时序图数据集上，StreamTGN相比现有方法实现了4.5倍至739倍的加速，且完全没有精度损失。
conclusion: StreamTGN通过减少冗余计算和优化GPU资源利用，为大规模动态图的实时推理提供了高效的解决方案。
---

## 摘要
时序图神经网络（TGNs）在动态图任务上取得了最先进的性能，然而现有系统仅专注于加速训练——在推理阶段，尽管只有一小部分节点受到影响，但每条新边都会触发 $O(|V|)$ 次嵌入更新。我们提出了 StreamTGN，这是首个利用时序图更新固有局部性的流式 TGN 推理系统：在 $L$ 层 TGN 中，一条新边仅影响端点 $L$ 跳范围内的节点，在百万级节点图中，这一比例通常低于 0.2%。StreamTGN 维护持久化的 GPU 驻留节点内存，并使用脏标记传播（dirty-flag propagation）来识别受影响集合 $\mathcal{A}$，在零精度损失的情况下将每批次的复杂度从 $O(|V|)$ 降低到 $O(|\mathcal{A}|)$。漂移感知的自适应重建调度和具有松弛排序的批处理流进一步最大化了吞吐量。在八个时序图（2K 至 2.6M 节点）上的实验表明，在精度相同的情况下，TGN 的加速比达到 4.5 倍至 739 倍，TGAT 的加速比高达 4,207 倍。StreamTGN 与训练优化是正交的：将 SWIFT 与 StreamTGN 结合，在三种架构（TGN、TGAT、DySAT）上实现了 24 倍的端到端加速。
