...
score: 6.0
evidence: 异构硬件间的资源分配
tldr: 本研究针对现代异构计算系统重新审视了阿姆达尔定律。传统定律仅关注串行与并行拆分，而现代AI工作负载受缩放法则驱动，涉及专用加速器与可编程计算的资源分配。作者提出了一种新的数学框架，揭示了当可扩展部分超过临界阈值时，专用硬件的效率优势将消失，导致投资转向通用计算。该理论解释了为何GPU在AI领域依然占据主导地位，而非被专用加速器完全取代。
selection_source: fresh_fetch
motivation: 传统的阿姆达尔定律无法准确描述现代异构硬件与AI缩放法则下的资源分配与效率权衡。
method: 重新构建了适用于异构系统和可扩展工作负载的阿姆达尔定律数学模型。
result: 发现了一个“崩溃阈值”，即当工作负载的可扩展比例超过临界点时，专用化投资的最优值会发生相变并降至零。
conclusion: 该框架解释了GPU可编程性增强的趋势，并阐明了专用AI加速器未能取代GPU的深层架构原因。
---
