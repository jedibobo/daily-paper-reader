...
## 摘要
伊辛模型最初于一个世纪前提出，在近几十年中已成为组合优化的基石。然而，伊辛机仍受限于基本的硬件-速度权衡。我们引入了 Bounce-Bind 伊辛机（BBIM），这是一种具有单个可调参数的机制，可在不改变能量景观的情况下调节自旋动力学。该机制基于经典的“高尔夫球”类比，但将其替换为动态的“网球/铅球”系统。Bounce 模式（加速逃离局部极小值）和 Bind 模式（实现快速收敛）动态地平衡了速度与质量。在稠密 MAX-CUT（边密度=0.5）基准测试中，BBIM 在 n=200 时实现了 6.15 倍的峰值加速。对于稀疏的 3-正则 3-XORSAT（二阶），在 n=160 时峰值加速达到 27.3 倍。两种结果所产生的额外硬件资源消耗均可忽略不计。这项工作展示了规避硬件-速度瓶颈的关键途径及其在大规模优化硬件上的实际适用性，并在结构迥异的基准测试中得到了验证。
