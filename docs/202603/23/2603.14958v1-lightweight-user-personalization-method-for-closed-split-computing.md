...
motivation: 旨在解决封闭拆分计算环境下，由于模型参数不可访问及环境动态变化导致的推理性能退化问题。
method: 通过在客户端部署轻量级适配器来精炼冻结头部网络的中间特征，实现多目标的模型自适应而无需改动主干网络。
result: "在CIFAR-10上将准确率提升至93.8%并降低60%以上的训练延迟，同时在极端丢包和噪声干扰下保持了高鲁棒性。"
conclusion: SALT框架证明了通过轻量级中间表示微调，可以在不增加通信负担的前提下有效提升拆分计算系统的实用性与效率。
---
