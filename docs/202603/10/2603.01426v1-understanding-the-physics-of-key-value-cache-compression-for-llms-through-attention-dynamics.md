...
method: 提出物理启发的KV压缩视角，利用多实体追踪和多跳推理等合成任务，分析不同架构在压缩下的保留、可达与利用情况。
result: "发现适度压缩揭示了冗余，但在90%压缩率附近存在性能相变，且不同架构在注意力路由的共识与多样性上表现出截然不同的动态特征。"
conclusion: KV缓存压缩可作为探测注意力几何结构的工具，其容忍度受稀疏路由结构支配，将长文本扩展性与自注意力的彩票假设联系起来。
---

## 摘要
随着大语言模型（LLM）的上下文窗口扩展到 100K+ token，键值（KV）缓存成为主要的内存瓶颈，近期的方法声称可以节省 80-90% 的内存且基准测试性能下降极小。我们认为这些评估忽略了一个结构性问题：注意力不仅是存储，更是路由，保留 KV 对并不能保证语义的可访问性。我们提出了一种受物理学启发的观点，将 KV 压缩视为对 token 级路由的受控扰动，并区分了保留、可访问性和利用率。通过探测多实体追踪、消歧、指代消解和多步推理的合成任务，我们发现：中度压缩会降低内部表示的质量，但准确率损失很小，这揭示了冗余性；所有模型在 90% 压缩率附近都表现出剧烈的幻觉安全悬崖，这与全局剔除率（GER）的激增相关，暗示了语义可达性的相变；不同架构的路由动力学存在差异，LLaMA 表现出早期共识和后期多样化，而 Qwen 表现出漏斗状的后期收敛，从而导致了不同的韧性特征。除了擦除之外，我们还发现了表示僵化现象，即尽管 token 依然存在，但过度的多头共识会导致路由灵活性崩溃。这些结果表明，稀疏的 token 路由结构决定了压缩容忍度，将 KV 压缩重新定义为注意力几何的结构性探测，并将长上下文的可扩展性与自注意力中的稀疏性和彩票假设联系起来。
