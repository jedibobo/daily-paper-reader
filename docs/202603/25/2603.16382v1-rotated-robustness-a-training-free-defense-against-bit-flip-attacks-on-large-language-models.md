...
tldr: 本研究针对大语言模型在量化权重位翻转攻击下的脆弱性，提出了一种名为Rotated Robustness (RoR) 的无需训练的防御方法。研究发现，模型崩溃源于激活值离群点与敏感权重位的空间对齐。RoR通过引入正交Householder变换旋转激活空间，从几何上平滑离群点，打破了这种对齐关系。该方法在数学上保证了原始精度，在Llama和Qwen等模型上表现优异，能将针对性攻击的复杂度提升数千倍，且仅带来极低的存储和推理开销。
selection_source: fresh_fetch
motivation: 硬件故障导致的量化权重位翻转会因激活值离群点而被放大，从而引发大语言模型灾难性的性能崩溃。
method: 提出RoR防御方案，利用正交Householder变换旋转激活空间，通过几何平滑离群点来打破其与敏感权重的空间对齐。
result: "RoR在保持原始精度的同时，将随机翻转崩溃率降至零，并将针对性攻击所需的翻转位数从个位数提升至超过17,000个。"
conclusion: RoR是一种无需训练、无损精度且计算开销极低的防御机制，为大语言模型的安全部署提供了高效可靠的保障。
---

## 摘要
硬件故障，特别是量化权重中的比特翻转，对大语言模型（LLMs）构成了严重的可靠性威胁，通常会引发灾难性的模型崩溃。我们证明了这种脆弱性从根本上源于敏感权重位与极端激活离群值（outliers）之间的空间对齐，这导致单个硬件故障被大幅放大。为了解决这一问题，我们提出了旋转鲁棒性（Rotated Robustness, RoR），这是一种利用正交 Householder 变换的免训练防御方法。通过对激活空间应用正交旋转，RoR 在几何上平滑了所有特征维度上的极端离群值。该机制有效地打破了离群值与易受攻击权重之间的对齐，并在数学上保证了原始模型的准确率。在 Llama-2/3、OPT 和 Qwen 系列模型上的广泛实证评估证明了我们方法卓越的可靠性。在随机比特翻转攻击下，RoR 将 Qwen2.5-7B 的随机崩溃率从 3.15% 降低到 0.00%。此外，在包含 50 次渐进式比特搜索（Progressive Bit Search）翻转的严峻针对性攻击下，RoR 在 Llama-2-7B 上维持了鲁棒的推理能力，保持了 43.9% 的 MMLU 准确率，与未受攻击时的 45.2% 几乎持平，而竞争防御方法则崩溃至随机猜测水平。最值得注意的是，针对单点故障攻击（SPFA）——这一最具攻击性的针对性威胁——RoR 将攻击复杂度从几个比特指数级增加到超过 17,000 个精确比特翻转。凭借在 Llama-2-7B 上仅 0.31% 的微小存储开销和 9.1% 的极低推理延迟增加，RoR 实现了真正的无损鲁棒性，为大语言模型部署提供了一种实用且高度可靠的防御方案。
