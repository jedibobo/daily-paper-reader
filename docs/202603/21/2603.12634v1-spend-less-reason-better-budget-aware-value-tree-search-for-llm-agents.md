...
tags: ["query:hwnas"]
score: 6.0
evidence: 针对计算缩放的预算感知推理时间框架
tldr: 针对LLM智能体在推理时过度消耗资源的问题，本文提出BAVT框架。这是一种无需训练的推理时搜索方法，通过动态搜索树和步级价值评估引导推理。其核心创新在于预算感知的节点选择机制和残差价值预测器，实现了从探索到利用的平滑过渡，并有效剪枝冗余步骤。实验证明，BAVT在低预算下表现优于4倍资源的基线方法，显著提升了推理效率。
selection_source: fresh_fetch
motivation: 现有的LLM推理增强方法往往忽略计算成本，导致在冗余步骤或错误路径上浪费大量Token和工具调用。
method: 提出BAVT框架，利用预算感知的节点选择机制和残差价值预测器，在单一LLM上实现动态搜索与智能剪枝。
result: 在多个多跳问答基准测试中，BAVT在低预算约束下的表现超过了分配4倍资源的并行采样基线。
conclusion: 研究证明智能的预算管理比单纯增加计算规模更能有效提升LLM智能体的推理性能和资源利用率。
---
