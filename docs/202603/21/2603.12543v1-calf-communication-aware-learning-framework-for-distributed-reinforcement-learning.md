...
motivation: 传统的强化学习训练假设零延迟交互，导致策略在存在网络波动的现实分布式环境中性能严重下降。
method: 提出CALF框架，在仿真训练阶段显式建模并引入延迟、抖动和丢包等真实网络约束。
result: 实验表明，与忽略网络因素的基准相比，CALF显著减少了部署时的性能损失，并在异构硬件上验证了其鲁棒性。
conclusion: 网络通信条件是分布式强化学习从仿真到现实迁移的重要影响因素，必须在训练中予以显式建模。
---

## 摘要
分布式强化学习策略在边缘设备和云服务器之间部署时，面临网络延迟、抖动和丢包问题。标准强化学习训练假设交互为零延迟，这导致在现实网络条件下性能严重下降。我们提出了 CALF（通信感知学习框架），该框架在模拟过程中根据现实网络模型训练策略。系统性实验表明，与不考虑网络的基准模型相比，网络感知训练显著缩小了部署性能差距。在异构硬件上的分布式策略部署验证了在训练期间显式建模通信约束能够实现稳健的现实世界执行。这些发现确立了网络条件是类 Wi-Fi 分布式部署中“模拟到现实”（sim-to-real）迁移的一个主要维度，是对物理和视觉领域随机化的补充。
