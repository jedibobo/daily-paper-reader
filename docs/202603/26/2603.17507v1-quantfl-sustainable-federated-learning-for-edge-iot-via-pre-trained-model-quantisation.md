...
## 摘要
联邦学习 (FL) 能够在物联网 (IoT) 设备上实现隐私保护的智能，但由于频繁上行传输的高能耗，导致了显著的碳足迹。尽管预训练模型在边缘设备上已日益普及，但其在降低微调能耗开销方面的潜力仍未得到充分探索。在这项工作中，我们提出了 QuantFL，这是一个可持续的联邦学习框架，它利用预训练初始化来实现激进且计算轻量级的量化。我们证明了预训练能自然地使更新统计数据集中，从而使我们能够使用内存高效的分桶量化 (bucket quantisation)，而无需复杂误差反馈机制带来的高能耗开销。在 MNIST 和 CIFAR-100 数据集上，QuantFL 在严格的带宽预算下，在匹配或超过未压缩基准性能的同时，减少了 40% 的总通信量（全精度下行链路时总比特数减少约 40%；在上行链路或下行链路量化时减少 ≥80%）；BU 在比特数减少几个数量级的情况下，分别达到了 89.00% (MNIST) 和 66.89% (CIFAR-100) 的测试准确率。我们还考虑了上行和下行链路的成本，并对量化水平和初始化进行了消融实验。QuantFL 为受电池限制的物联网网络上的可扩展训练提供了一种实用的“绿色”方案。
