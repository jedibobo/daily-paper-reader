...
tldr: 针对现代大规模电路图中深度图神经网络（GNN）训练面临的显存瓶颈和高昂计算成本，本文提出了GSR-GNN框架。该框架结合了可逆残差模块、分组稀疏非线性算子及优化的执行流水线，支持训练数百层的深度GNN。在保持预测精度的同时，显著降低了显存占用并提升了训练速度，使深度GNN在EDA领域的大规模应用成为可能。
...
result: "在电路图样本上实现了高达87.2%的峰值显存缩减和超过30倍的训练加速，且预测精度几乎无损。"
conclusion: GSR-GNN有效解决了深度GNN在EDA任务中的扩展性难题，为大规模电路分析提供了高效的训练方案。
---

## 摘要
图神经网络 (GNN) 在电路分析中展现出巨大的潜力，但扩展到现代大规模电路图时受到 GPU 显存和训练成本的限制，尤其是对于深层模型而言。我们重新审视了用于电路图的深层 GNN，并表明在可训练的情况下，它们的性能显著优于浅层架构，这促使我们开发一种高效的领域特定训练框架。我们提出了分组稀疏可逆 GNN (GSR-GNN)，它能够训练多达数百层的 GNN，同时降低计算和内存开销。GSR-GNN 将可逆残差模块与分组稀疏非线性算子相结合，在不牺牲任务相关信息的情况下压缩节点嵌入，并采用优化的执行流水线来消除碎片化的激活存储并减少数据移动。在采样的电路图上，GSR-GNN 实现了高达 87.2% 的峰值显存降低和超过 30 倍的训练加速，且基于相关性的质量指标下降微乎其微，使得深层 GNN 在大规模 EDA 工作负载中具有实用性。
