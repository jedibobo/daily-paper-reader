...
tldr: 针对稀疏混合专家模型（MoE）推理时内存开销巨大的挑战，本文提出了一种具有理论泛化保障的专家级混合精度量化策略。该方法通过分析训练过程中路由器L2范数的变化以及神经元内部方差，识别出对模型性能更敏感的关键专家并为其分配更高位宽。在Switch Transformer和Mixtral等模型上的实验证明，该方法在显著降低推理成本的同时，以极低的分配开销实现了优于现有方法的准确率。
selection_source: fresh_fetch
motivation: 现有的MoE量化方法在低位宽下精度损失严重，且混合精度分配策略计算开销大，忽略了不同专家对量化敏感度的显著差异。
method: 提出一种基于理论依据的专家级混合精度策略，依据路由器L2范数的变化量和神经元内最大方差来动态分配各专家的量化位宽。
result: 在Switch Transformer和Mixtral等大规模模型上的实验表明，该方法在保持高精度的同时显著降低了推理成本，且位宽分配过程的额外开销几乎可以忽略。
conclusion: 通过结合路由动态和神经元统计特性，可以实现高效且具有泛化保障的MoE模型量化，为大规模稀疏模型的高效部署提供了新方案。
---
