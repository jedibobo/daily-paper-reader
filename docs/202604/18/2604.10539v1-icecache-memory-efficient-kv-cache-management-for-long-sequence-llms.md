...
tldr: "针对长序列大语言模型推理中KV缓存占用内存过大的问题，本文提出IceCache管理策略。该方法结合语义令牌聚类与PagedAttention，通过分层动态数据结构组织语义相关令牌，优化了CPU-GPU间的传输效率与令牌选择精度。在LongBench测试中，IceCache仅需25%的缓存预算即可达到甚至超越现有方法的性能，在极低预算下仍能保持99%的原始精度。"
selection_source: fresh_fetch
motivation: 现有的KV缓存卸载技术在长序列任务中存在令牌选择不精准和内存带宽利用率低导致的性能下降问题。
method: 提出IceCache策略，利用语义令牌聚类和分层动态数据结构，将语义相关的令牌组织在连续内存区域中进行高效管理。
result: "在LongBench上，IceCache仅用256个令牌预算就维持了99%的原始精度，且在仅使用25%预算的情况下，延迟和准确率均优于同类卸载方法。"
conclusion: IceCache通过高效的语义聚类管理，显著提升了长序列LLM在资源受限硬件上的推理效率和准确性。
---
