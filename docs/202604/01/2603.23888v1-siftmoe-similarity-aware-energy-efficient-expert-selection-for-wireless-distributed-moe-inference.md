...
title_zh: SiftMoE：面向无线分布式 MoE 推理的感知相似性且高能效的专家选择
...
## 摘要
混合专家（MoE）架构利用稀疏激活来增强大语言模型（LLM）的可扩展性，使其适用于部署在资源受限的边缘网络中。然而，庞大的专家数量往往超出了单个边缘节点的内存容量，因此需要进行无线分布式 MoE（WIDE）推理，将专家分布在多个边缘节点上。在这种背景下，专家选择直接影响通信成本。受专家相似性的启发，我们提出了 SiftMoE，它通过明智地选择或跳过专家，在通信成本和推理准确性之间取得权衡。具体而言，我们首先建立了由专家替换或跳过导致的准确性下降的理论界限。基于这些界限，我们在时延和准确性约束下，为 WIDE 推理中的专家选择构建了一个能量最小化问题。特别地，针对慢衰落信道，我们推导了单令牌解码和多令牌预填充的最优专家选择策略。针对快衰落信道，我们进一步扩展了方案，以应对快速变化的信道条件。仿真结果表明，与 WIDE 系统中传统的 Top-K 路由相比，SiftMoE 在保持推理准确性的同时显著降低了能耗。
