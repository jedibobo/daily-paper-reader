...
tldr: "针对分布式AI推理开发中缺乏标准化建模工具、实验难以复现等问题，本文提出了UNIFERENCE离散事件仿真框架。该框架通过轻量级逻辑进程模拟异构设备与网络行为，并与PyTorch Distributed无缝集成，支持同一代码库从仿真到部署的平滑过渡。实验表明其运行时间预测准确率高达98.6%，为分布式推理算法的研究和未来系统设计提供了高效、可复现的平台。"
selection_source: fresh_fetch
motivation: 现有的分布式推理研究依赖特定测试平台或私有设施，导致结果难以复现且难以探索假设性的硬件配置。
method: 提出一种基于离散事件仿真的框架，利用轻量级逻辑进程同步通信原语，并支持与PyTorch分布式环境的无缝切换。
result: "在多种后端和硬件设置下，UNIFERENCE的运行时间分析准确率与实际物理部署相比高达98.6%。"
conclusion: UNIFERENCE弥合了仿真与部署之间的鸿沟，为从高性能集群到边缘设备的分布式AI模型研究提供了统一且开源的工具。
---

## 摘要
由于缺乏对异构设备和网络建模的标准工具，开发和评估分布式推理算法仍然具有挑战性。现有研究通常依赖于临时测试床或专有基础设施，导致结果难以复现，并限制了对假设性硬件或网络配置的探索。我们提出了 UNIFERENCE，这是一个离散事件仿真（DES）框架，旨在统一环境中开发、基准测试和部署分布式人工智能模型。UNIFERENCE 通过轻量级逻辑进程对设备和网络行为进行建模，这些进程仅在通信原语上进行同步，在消除回滚的同时保留了因果顺序。它与 PyTorch Distributed 无缝集成，使同一代码库能够从仿真平滑过渡到实际部署。我们的评估表明，在不同的后端和硬件设置下，与实际物理部署相比，UNIFERENCE 分析运行时间的准确率高达 98.6%。通过桥接仿真与部署，UNIFERENCE 为研究分布式推理算法和探索从高性能集群到边缘规模设备的未来系统设计提供了一个易于访问且可复现的平台。该框架已在 https://github.com/Dogacel/Uniference 开源。
