...
## 摘要
全同态加密 (FHE) 近年来作为一种密码学原语和系统挑战引起了广泛关注。鉴于加速计算的最新进展，FHE 为技术进步提供了充满前景的机遇，其应用范围涵盖从机器学习到信息安全的多个领域。我们从硬件角度出发，针对深度神经网络中计算最密集的算子——矩阵乘法 (matmul)，并将其适配到 AMD GPU 上执行。我们提出了一种新的优化方法，通过使用专门为 GPU 设计的最新开源 FHE 库 FIDESlib，提升了密文矩阵乘法的运行速度并降低了其复杂度。通过利用两个操作数的稀疏性，我们的稀疏矩阵乘法实现比对应的 CPU 实现性能提升高达 3.0 倍，并将时间复杂度从立方降低到拟线性，展示了对现有 FHE 矩阵乘法实现的改进。
