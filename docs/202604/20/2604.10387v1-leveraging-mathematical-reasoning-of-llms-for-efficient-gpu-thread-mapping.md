...
tldr: 本研究针对GPU计算中非矩形区域线程映射的难题，提出利用大语言模型（LLM）的符号推理能力自动推导映射函数。通过对开源模型（如DeepSeek-R1）的评估，证明其能生成高效的O(1)或O(log N)映射方程。尽管模型推理过程耗能，但生成的内核在实际GPU负载中可实现数千倍的加速和节能，显著优于传统符号回归方法，为自动化GPU资源优化开辟了新路径。
