...
tldr: 针对图卷积网络（GCN）中稀疏矩阵乘法（SpMM）因节点度数幂律分布导致的不规则计算挑战，本文提出FlexVector向量处理器架构。该架构采用行优先乘积数据流以规范化执行并提升并行度，引入软件管理的灵活向量寄存器堆（VRF）适应不规则访问，并结合图感知预处理策略优化负载。实验表明，FlexVector在处理真实世界GCN负载时，相比传统缓存中心化架构显著提升了性能与能效。
selection_source: fresh_fetch
motivation: 现有加速器难以高效处理GCN推理中由节点度数幂律分布引起的不规则计算负载和内存访问模式。
method: 采用行优先数据流与灵活向量寄存器堆（VRF）的软硬件协同设计，配合图感知预处理和节点划分策略来优化SpMM执行。
result: "在五个真实数据集上，FlexVector相比最先进的缓存中心化基准架构实现了3.78倍的加速，并降低了40.5%的能耗。"
conclusion: 通过灵活的向量寄存器设计与图感知优化，FlexVector有效解决了变稀疏度图计算中的不规则性问题，是高效的GCN加速方案。
---
