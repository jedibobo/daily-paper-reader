...
title_zh: RF-LEGO：通过深度展开实现的射频感知模块化信号处理-深度学习协同设计
...
result: 在Wi-Fi、毫米波、UWB和6G等多种真实场景下的实验证明，RF-LEGO在独立任务和下游集成任务中均显著优于传统算法和深度学习基准。
conclusion: 该研究开创了无线感知中信号处理与深度学习协同设计的新范式，为构建高效、可解释且模块化的深度感知方案提供了重要参考。
---

## 摘要
无线感知传统上依赖于信号处理（SP）技术，近期已转向数据驱动的深度学习（DL）以实现性能突破。然而，现有的深度无线感知模型通常是端到端且针对特定任务的，缺乏可重用性和可解释性。我们提出了 RF-LEGO，这是一个模块化的协同设计框架，通过深度展开将可解释的信号处理算法转化为可训练且具有物理依据的深度学习模块。通过将手动调节的参数替换为可学习参数，同时保留核心处理结构和数学算子，RF-LEGO 确保了模块化、可级联性以及与结构对齐的可解释性。具体而言，我们针对关键的射频感知任务引入了三个深度展开模块：频率变换、空间角度估计和信号检测。利用 Wi-Fi、毫米波、超宽带（UWB）和 6G 感知的真实世界数据进行的广泛实验表明，无论是在独立运行还是集成到多个下游任务中时，RF-LEGO 的性能都显著优于现有的信号处理和深度学习基准。RF-LEGO 开创了一种通过深度展开实现无线感知信号处理-深度学习协同设计的新范式，为高效且可解释的深度无线感知解决方案提供了启示。我们的代码可在 https://github.com/aiot-lab/RF-LEGO 获取。
