...
tldr: 针对计算密集型AI任务在机器人领域的应用，本文提出一种自适应多接入边缘架构。通过集成基于MAPE-K循环的智能代理，在Kubernetes编排的异构边缘系统中实现动态扩展和任务卸载。该方案利用神经网络预测人类行为以优化机器人路径规划，在保证安全的同时显著提升了服务质量和能效。
selection_source: fresh_fetch
motivation: 随着AI任务计算需求增加，亟需降低处理成本并提高边缘计算环境下的性能与能效。
method: 构建了一个基于Kubernetes的异构分布式边缘卸载系统，并采用MAPE-K模型作为自适应监督器来管理资源缩放和任务分配。
result: 实验结果表明，该自适应架构在响应时间和功耗方面均优于传统配置，显著提升了服务质量。
conclusion: 本研究证明了自适应边缘架构在处理AI驱动的复杂机器人任务中具有显著的有效性和优越性。
---

## 摘要
计算密集型 AI 任务的增长凸显了降低处理成本、提高性能和能源效率的需求。这使得集成智能代理作为架构自适应主管变得至关重要，其任务是负责基础设施的自适应扩展以及连续体（continuum）内的高效计算卸载。本文提出了一种针对人机混合环境高效计算系统的自适应方法。计算任务涉及一种神经网络算法，该算法利用传感器数据预测人类移动行为，从而增强移动机器人的主动路径规划并确保人类安全。为了优化神经网络处理，我们构建了一个由 Kubernetes 编排的、包含异构处理单元的分布式边缘卸载系统。通过监测响应时间和功耗，基于 MAPE-K 的自适应主管在扩展和卸载方面做出明智决策。结果表明，与传统设置相比，该方法在服务质量方面有显著提升，证明了所提方法在 AI 驱动系统中的有效性。
