...
method: 设计了一种包含一等公民硬件原语、强化类型系统（支持 CDC/RDC 检查）以及 AI 友好型语法（无宏、LL(1) 文法）的新型硬件描述语言。
result: 通过 L1 缓存和 AXI DMA 控制器等案例验证，Arch 能在仿真前捕获多种设计缺陷，并生成符合工业标准的 SystemVerilog 代码。
conclusion: Arch 证明了通过结合严谨的类型安全机制与 AI 原生语法设计，可以显著提升硬件设计的开发效率与可靠性。
---

## 摘要
我们提出了 Arch（AI 原生寄存器传输级时钟硬件），这是一种从第一性原理出发设计的硬件描述语言，旨在用于微架构规范和 AI 辅助代码生成。Arch 为流水线、有限状态机 (FSM)、FIFO、仲裁器、寄存器堆、总线和跨时钟域 (CDC) 引入了一等语言结构，而现有的硬件描述语言 (HDL) 仅将这些结构表达为容易产生细微错误的用户定义模式。一个核心设计选择是，时钟和复位本身是参数化类型（Clock<D>, Reset<S,P,D?>）而非普通连线，从而将跨时钟域 (CDC) 和跨复位域 (RDC) 分析从外部 Linter 检查转换为编译时类型规则。结合对位宽、端口方向、单驱动所有权和组合逻辑无环性的同步跟踪，该类型系统在任何仿真器运行之前，就能捕获多驱动、未驱动端口、隐式锁存器、位宽不匹配、组合逻辑环路以及未同步的跨域问题。每一个语法选择都受“AI 可生成性契约”约束：LL(1) 文法无需回溯或多标记前瞻，不含预处理器或宏，采用统一的声明模式、命名的代码块结尾、显式的方向连接箭头以及 todo! 逃生舱口，使大语言模型 (LLM) 无需微调即可根据自然语言规范生成结构正确且类型安全的 Arch 代码。Arch 编译器输出确定性的、符合 Lint 规范的 IEEE 1800-2017 SystemVerilog，并提供集成仿真工具链，可生成用于周期精确仿真的编译后 C++ 模型。我们展示了一个 8 路组相联 L1 数据缓存和一个可综合的 PG021 兼容 AXI DMA 控制器（包含在 Sky130 上的 Yosys 和 OpenSTA 结果）的案例研究，并在表达能力、安全性和 AI 适用性维度上将 Arch 与 SystemVerilog、VHDL、Chisel、Bluespec 及其他现代 HDL 进行了对比。
