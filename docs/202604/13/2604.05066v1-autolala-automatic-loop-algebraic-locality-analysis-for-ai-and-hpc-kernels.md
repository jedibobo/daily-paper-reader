...
tldr: AutoLALA是一个针对AI和HPC内核（如矩阵乘法、张量收缩等）的开源数据局部性分析工具。它通过将仿射循环程序转化为多面体模型，利用符号化分析方法（如Barvinok计数）生成重用距离和数据移动复杂度的闭式符号公式。该工具采用Rust编写，避免了传统的堆栈模拟，为优化现代计算系统中的数据传输瓶颈提供了高效的自动化分析手段。
selection_source: fresh_fetch
motivation: 现代计算系统中数据移动成本已成为主要瓶颈，亟需一种能自动分析仿射循环程序数据局部性的符号化工具。
method: 基于多面体模型和Barvinok计数，将DSL编写的程序映射为时间戳空间和访问映射，从而推导出重用距离的闭式公式。
result: 成功实现了支持张量收缩、einsum和模板计算等多种复杂负载的自动化分析，并提供命令行及交互式Web界面。
conclusion: AutoLALA通过符号化代数分析，为理解和优化复杂循环嵌套的数据移动提供了高效且精确的理论支撑与实用工具。
---

## 摘要
数据移动是现代计算系统的主要瓶颈。对于高性能计算 (HPC) 和 AI 工作负载中常见的基于循环的程序（包括矩阵乘法、张量收缩、模板计算和 einsum 操作），数据在存储层级中移动的开销通常超过了算术运算的开销。本文介绍了 AutoLALA，这是一款用于分析仿射循环程序中数据局部性的开源工具。该工具接受由小型领域特定语言 (DSL) 编写的程序，将其降级为多面体集合与映射，并生成关于重用距离（reuse distance）和数据移动复杂度的闭式符号公式。AutoLALA 实现了 Zhu 等人的全符号局部性分析，并结合了 Smith 等人的数据移动距离 (DMD) 框架。特别地，它将重用距离计算为访问空间在访问映射下的像，从而避免了栈模拟和 Denning 的递归工作集公式。我们描述了 DSL 语法及其形式语义、通过仿射变换构建时间戳空间和访问映射的多面体降级流水线，以及用于推导符号化重用间隔和重用距离分布的一系列 Barvinok 计数操作。该系统采用 Rust 实现，是一个包含三个 crate 的模块化库，并提供了对 Barvinok 库的安全绑定。我们提供了一个命令行界面和一个带有输出公式 LaTeX 渲染功能的交互式 Web 游乐场。该工具可处理任意仿射循环嵌套，涵盖了张量收缩、einsum 表达式、模板计算和通用多面体程序等工作负载。
