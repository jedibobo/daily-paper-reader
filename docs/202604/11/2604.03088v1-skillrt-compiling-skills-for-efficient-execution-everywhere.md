...
title_zh: SkillRT：为跨平台高效执行而编译技能
...
tldr: "SkillRT是一个针对大模型智能体技能执行的编译与运行时系统。针对现有系统将技能视为原始上下文导致的跨平台不一致和低效问题，该研究借鉴传统编译器设计，将技能视为代码并将LLM视为异构处理器。通过能力分解、环境绑定、并发提取及JIT代码固化等技术，SkillRT实现了技能的可移植与高效执行。实验表明，它显著提升了任务完成率，减少了高达40%的Token消耗，并实现了最高50倍的延迟降低。"
selection_source: fresh_fetch
motivation: 现有LLM智能体将技能视为原始上下文，导致同一技能在不同模型和环境下表现不一致且执行效率低下。
method: 借鉴编译器设计，将技能分解为原子能力，通过编译时的环境绑定与并发提取，以及运行时的JIT代码固化与自适应重编译来优化执行。
result: "在多种模型和环境下，SkillRT显著提升了任务成功率，降低了40%的Token消耗，并实现了3.2倍的加速和最高50倍的延迟缩减。"
conclusion: 将技能视为可编译代码并结合运行时优化，是实现LLM智能体技能跨平台移植与高效执行的有效途径。
---

## 摘要
LLM 智能体越来越多地采用技能（skills）作为可重用的组合单元。尽管技能在不同的智能体平台之间共享，但现有系统将其视为原始上下文，导致同一技能在不同智能体上的表现不一致。这种脆弱性损害了技能的可移植性和执行效率。为了应对这一挑战，我们分析了 118,000 个技能，并从传统的编译器设计中汲取灵感。我们将技能视为代码，将 LLM 视为异构处理器。为了使可移植性具有可操作性，我们将技能的需求分解为一组原语能力（primitive capabilities），并衡量每个“模型-框架”（model-harness）组合对这些能力的支持程度。基于这些能力画像，我们提出了 SkillRT，这是一个专为可移植且高效的技能执行而设计的编译与运行时系统。在编译时，SkillRT 执行基于能力的编译、环境绑定和并发提取。在运行时，SkillRT 应用即时（JIT）代码固化和自适应重编译来进行性能优化。我们在八种不同规模的 LLM 和三个智能体框架上评估了 SkillRT，涵盖了 SkillsBench 和具有代表性的技能任务。结果表明，SkillRT 显著提高了不同模型和环境下的任务完成率，同时将 Token 消耗降低了高达 40%。在性能方面，SkillRT 通过增强并行性实现了高达 3.2 倍的加速，并通过代码固化实现了 19-50 倍的延迟降低。
