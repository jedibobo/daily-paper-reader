...
tldr: 针对企业部署小语言模型（SLM）时推理能力不足且大模型成本高昂的问题，本文提出语义梯度下降（SGDe）框架。该框架利用大模型作为教师，通过自然语言反馈迭代优化SLM的执行计划，将其编译为包含DAG拓扑、系统提示词和确定性代码的智能体工作流。SGDe在极小样本下表现优异，显著提升了SLM在复杂推理任务中的准确率与可靠性。
selection_source: fresh_fetch
motivation: 解决小语言模型推理错误难以自纠，而顶级大模型在企业高频使用中面临成本与数据主权限制的问题。
method: 提出SGDe框架，利用教师模型生成的自然语言评价作为“语义梯度”，将任务编译为结合确定性代码和结构化共识的DAG工作流。
result: "在GSM-Hard衍生测试集上，该方法在极小样本下实现了91.3%至99.3%的准确率，较现有提示词优化器提升了26.3%至34.3%。"
conclusion: 通过将确定性结构编译进SLM外壳，SGDe成功实现了复杂推理任务从昂贵大模型向高效小模型的可靠迁移。
---

## 摘要
企业部署小语言模型 (SLM) 受到认知不对称性的限制：SLM 无法自我纠正推理错误，而前沿大语言模型 (LLM) 成本极高，且在大规模使用时面临数据主权限制。我们提出了语义梯度下降 (SGDe)，这是一种教师-学生框架，可将智能体工作流编译为由 DAG 拓扑、系统提示词和确定性可执行代码组成的离散执行计划。末尾的“e”将 SGDe 与随机梯度下降 (SGD) 区分开来。SGDe 在离散语义空间中运行，其中前沿教师模型生成自然语言评论，作为方向梯度来迭代优化 SLM 的工作流构件。我们在 PAC 学习框架内对 SGDe 进行了形式化，建立了样本复杂度界限，通过利用教师模型作为统计先验，在目标合成任务上仅需三个训练样本即可实现收敛。在通过对抗性合成构建的 GSM-Hard 衍生测试集上，在受推论 1 启发的“小 m”机制下，编译后的工作流在 m=5 时达到 91.3% 的准确率，在 m=3 时达到 99.3%，相比最先进的提示词优化器实现了 +26.3% 至 +34.3% 的绝对提升。在兴起的框架工程 (harness engineering) 范式中，SGDe 将确定性代码的放置（哪些子任务委托给 Python 运行时，哪些保留为 LLM 调用）视为一种基于轨迹的逐节点优化目标，推广了 PAL 和 PoT 的全问题卸载模式。教师模型编译了两种互补的确定性结构：能力卸载（当 SLM 无法可靠执行子任务时将其委托给 Python）和结构共识（将方差受限的推理步骤封装在通过确定性投票聚合的扇出/扇入子图中）。
