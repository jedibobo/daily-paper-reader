...
tldr: 本文针对大视觉语言模型（LVLM）推理中存在的“视觉Token主导”效率瓶颈，系统性地梳理了涵盖编码、预填充和解码全生命周期的优化技术。文章分析了高分辨率特征提取、注意力机制缩放及内存带宽限制带来的挑战，提出了一个解耦信息密度、长上下文管理和内存极限的分类框架，并探讨了视觉保真度与系统效率之间的权衡，为未来硬件-算法协同设计提供了指导。
selection_source: fresh_fetch
motivation: 视觉Token的过度冗余导致LVLM推理面临严重的计算开销和内存带宽瓶颈。
method: 建立了一套基于推理生命周期的系统性分类法，深入分析了从视觉编码到解码各阶段的效率优化技术。
result: 揭示了上游编码决策对下游推理瓶颈的级联影响，并总结了平衡视觉保真度与系统性能的多种优化路径。
conclusion: 提出了混合压缩、模态感知解码、渐进式状态管理及阶段解耦服务等四个未来研究前沿方向。
---
