...
tldr: 针对图 Transformer 在大规模图上训练时面临的单机显存不足和效率低下问题，本文提出了一种可扩展且自适应的分布式训练框架。该框架能根据图结构和硬件配置自动优化并行策略，并实现了高效的分布式稀疏操作。实验表明，该框架显著降低了内存消耗并提升了训练速度，为构建大规模图基础模型提供了有力支持。
selection_source: fresh_fetch
motivation: 现有的图 Transformer 训练受限于单 GPU 显存和计算效率，难以处理大规模图数据并面临内存溢出挑战。
method: 提出一种分布式训练框架，通过自动选择并行策略和优化分布式稀疏操作来提升计算效率和内存利用率。
result: "在 8 GPU 环境下实现了高达 6 倍的加速，稀疏注意力计算提速 3.8 倍且内存消耗降低了 78%。"
conclusion: 该框架显著增强了图 Transformer 的可扩展性，使其能够高效地应用于大规模图基础模型的训练。
---
