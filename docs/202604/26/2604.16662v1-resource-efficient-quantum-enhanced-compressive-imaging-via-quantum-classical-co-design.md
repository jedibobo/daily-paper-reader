...
## 速览
**TLDR**：本研究针对传统量子成像中量子资源消耗随像素线性增长的问题，提出了一种量子-经典协同设计框架。通过将量子硬件层的资源分配与经典压缩成像软件层相结合，利用主成分分析（PCA）识别低维主成分子空间，并仅对最具信息量的空间模式应用量子压缩。该方法在显著减少量子资源需求的同时，实现了高精度的图像分类和高保真度重建，为高效量子增强成像提供了新范式。 \
**Motivation**：传统量子成像方案中量子资源消耗随图像维度线性增长，导致在大规模成像应用中成本过高且效率低下。 \
**Method**：提出一种协同设计框架，利用PCA识别图像的低维主成分子空间，并有针对性地对这些关键空间模式应用量子压缩。 \
**Result**：数值实验证明，该方法仅需极少量的压缩模式即可实现与逐像素压缩相当的高精度图像分类和高保真度重建。 \
**Conclusion**：量子硬件与经典算法的协同设计能显著提升量子成像的资源效率，为实用化量子增强传感奠定了基础。
