...
tldr: 针对物理神经网络难以实现反向传播（BP）和自动微分（AD）的问题，本文提出了一种名为FFzero的纯前向学习框架。该框架结合了逐层局部学习、原型表示和基于方向导数的优化，实现了无需BP的稳定训练。FFzero适用于MLP和CNN，并在光子神经网络仿真中证明了其作为原位物理学习路径的可行性，有效降低了对数字计算的依赖。
selection_source: fresh_fetch
motivation: 传统反向传播算法难以在物理硬件中直接实现，限制了物理神经网络的能效和原位训练能力。
method: 提出FFzero框架，通过逐层局部学习、原型表示及基于前向评估的方向导数优化，实现无需反向传播的训练。
result: 实验证明FFzero在分类和回归任务中对MLP和CNN均有效，且在BP失效的纯前向优化场景下表现出更强的稳定性。
conclusion: FFzero为物理神经网络提供了一种高效、稳定的原位学习方案，推动了摆脱数字计算依赖的物理学习发展。
---
