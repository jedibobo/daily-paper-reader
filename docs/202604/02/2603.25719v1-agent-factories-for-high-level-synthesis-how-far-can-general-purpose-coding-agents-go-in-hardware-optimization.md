...
tldr: 本研究探讨了通用编程智能体在无需硬件特定训练的情况下，对高层次综合（HLS）硬件设计的优化能力。提出了一种“智能体工厂”两阶段流水线：第一阶段通过子内核分解与整数线性规划（ILP）进行初步优化与组合；第二阶段利用多个专家智能体进行跨函数全局优化。实验表明，该方法在多个基准测试中实现了显著的加速比，证明了智能体扩展是提升HLS性能的有效途径。
selection_source: fresh_fetch
motivation: 探索通用编程智能体在缺乏领域特定训练的情况下，能否通过自动化流水线有效优化复杂的硬件高层次综合设计。
method: 提出一种两阶段“智能体工厂”，结合子内核分解、ILP全局约束求解以及多智能体并行的跨函数全局优化探索。
result: 在12个基准测试中，通过扩展至10个智能体实现了平均8.27倍的加速，部分复杂任务加速比超过20倍，且智能体能自主发现经典硬件优化模式。
conclusion: 实验证明通过增加智能体数量进行规模化探索是解决HLS优化问题的实用且高效的路径。
---
