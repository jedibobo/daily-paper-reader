...
tldr: 针对动态边缘/雾计算网络中垂直联邦学习（VFL）面临的设备异构性和动态性挑战，本文提出了SC-DN框架。该框架通过联合优化服务器部署、传输功率、处理器频率和本地迭代次数，在保证模型收敛的同时最小化资源消耗。研究将该问题建模为混合整数符号规划并开发了通用求解器，实验证明其在分类和回归任务中均优于贪婪算法。
selection_source: fresh_fetch
motivation: 边缘网络中设备数据特征和硬件的异构性及动态变化，使得VFL的训练效率和资源分配面临巨大挑战。
method: 提出SC-DN方法，通过建立全局一阶驻点存在性，联合优化服务器位置、通信功率、计算频率及迭代次数，并求解复杂的混合整数符号规划问题。
result: 在图像和多模态数据集上的实验表明，该方法在提升模型性能的同时，显著降低了系统资源消耗。
conclusion: 本研究为动态边缘环境下的垂直联邦学习提供了有效的资源调度与模型训练协同优化方案。
---

## 摘要
我们研究了动态边缘/雾网络中纵向联邦学习（VFL）的控制与优化。VFL 是一类分布式机器学习（ML）方法，其中边缘/雾设备包含不同的数据特征。由于边缘/雾网络中异构的数据特征和硬件，设备对 VFL 的贡献差异巨大；此外，动态边缘/雾网络可能导致特定数据特征的永久退出或进入。在此背景下，我们提出的方法——动态网络中服务器控制的 VFL（SC-DN），首先确立了每一轮全局迭代中全局一阶驻点的存在性，然后利用该结果，基于四个关键控制变量联合优化 ML 模型训练和资源消耗：(i) 服务器部署，(ii) 设备到服务器的发射功率，(iii) 本地设备处理器频率，以及 (iv) 每轮全局迭代的本地训练次数。由此产生的优化公式包含耦合变量以及多种形式的对数约束，我们证明这是一个混合整数符号多项式规划（MISP）问题，属于 NP-hard 问题，并为此开发了一个通用求解器。最后，通过在图像和多模态数据集上的实验，我们证明了我们的方法在分类/回归性能和资源消耗节省方面均优于贪婪算法。
