...
title_zh: SWAN：针对运行时变化的自适应环境感知多模态网络
...
tldr: 针对多模态深度学习在实际部署中面临的模态质量波动、输入复杂度和计算资源限制等挑战，本文提出了SWAN。SWAN是首个能同时应对这三类变化的自适应网络，通过质量感知控制器根据预算分配资源，并结合自适应门控和令牌丢弃技术优化样本级效率。在自动驾驶3D检测任务中，SWAN在保持高性能的同时显著降低了计算开销，实现了资源利用的最大化。
selection_source: carryover_cache
motivation: 现有多模态网络无法同时兼顾严格的计算预算、动态的模态质量以及样本本身的复杂度变化，导致资源利用率低下。
method: 引入质量感知控制器分配模态资源，并利用自适应门控模块和令牌丢弃技术根据样本复杂度动态调整计算深度与特征密度。
result: "在自动驾驶3D目标检测实验中，SWAN在性能几乎不下降的前提下，成功将计算量（FLOPs）降低了高达49%。"
conclusion: SWAN证明了通过感知环境与样本特征的自适应机制，可以在资源受限且多变的运行环境下实现高效且稳健的多模态感知。
---

## 摘要
部署在现实环境中的多模态深度神经网络必须应对运行时变化：模态质量的变化、整体输入复杂度的变化以及可用平台资源的变化。目前的网络难以应对此类波动——自适应网络无法遵守严格的计算预算，基于控制器的网络忽略了输入复杂度，而静态配置的网络在上述所有方面都表现不佳。因此，它们无法从消耗的计算资源中提取最大效用。我们提出了 SWAN（样本与环境感知多模态网络），这是第一个同时实现上述三个目标的自适应多模态网络。SWAN 采用质量感知控制器，根据用户指定的动态最大预算在各模态之间分配资源。在此预算范围内，自适应门控模块通过根据样本复杂度缩放层利用率来进一步优化效率。为了进一步提升性能，SWAN 还采用了一个令牌丢弃模块，在执行检测之前掩盖语义无关的多模态特征。我们在自动驾驶领域的复杂多目标 3D 检测任务中评估了 SWAN，在性能下降极小的情况下，将 FLOPs 降低了高达 49%。
