...
tags: ["query:hwnas"]
score: 6.0
evidence: 通过流水线并行缓解内存和互连瓶颈
tldr: 在消费级GPU上微调大模型受限于显存和PCIe带宽，现有流水线并行方案因权重绑定导致严重的流水线气泡。本文提出RoundPipe，通过将GPU视为无状态执行单元并采用轮询调度，打破了权重绑定限制，实现了近乎零气泡的流水线。结合优先级感知传输和自动层划分，RoundPipe在8卡RTX 4090上实现了显著加速，并支持在单机上微调超大规模模型。
selection_source: fresh_fetch
motivation: 现有流水线并行方案因模型层负载不均导致的权重绑定问题，在低带宽消费级GPU环境下会产生严重的流水线气泡。
method: 提出RoundPipe调度算法，将GPU作为无状态工人进行轮询任务分发，并集成优先级感知传输引擎和自动分区算法。
result: 在8路RTX 4090服务器上，RoundPipe相比现有基准实现了1.48至2.16倍的加速，并成功支持了Qwen3-235B模型的微调。
conclusion: RoundPipe通过打破权重绑定约束，显著提升了在消费级硬件上进行大规模语言模型微调的效率和可行性。
---
