...
date: 2026-05-07
pdf: "https://arxiv.org/pdf/2605.05978v1"
tags: ["query:hwnas"]
score: 7.0
...
## 摘要
高容量联想记忆模型，如核逻辑回归 (KLR) Hopfield 网络，已展现出强大的存储能力，但通常依赖于计算成本高昂的同步更新。这种依赖性为在节能、事件驱动的类脑硬件上的部署构成了瓶颈。在本文中，我们研究了 KLR Hopfield 网络的异步检索动力学。我们通过实验表明，在适当调整核参数的情况下，异步顺序更新表现出的轨迹在统计上与同步动力学的轨迹不可区分，同时在随机模式的测试范围内保持了高召回准确率。此外，我们发现异步网络在静态随机模式方案中实现了接近 $P/N \approx 30$ 的经验存储容量，超过了经典极限。为了评估计算效率，我们分析了纠错所需的总状态转换（比特翻转）次数。结果表明，网络收敛所需的事件数量接近与目标模式的初始汉明距离，且没有观察到明显的伪振荡。这些发现表明，由 KLR 学习诱导的大边距吸引子创建了一个适合稀疏、事件驱动计算的光滑能量景观，为在类脑架构上实现可扩展且低功耗的联想记忆提供了基础。
