...
tags: ["query:hwnas"]
score: 6.0
evidence: 通过求解器收敛确定有效计算深度以避免不必要的计算
tldr: 针对深度联合源信道编码（Deep JSCC）中固定深度架构导致的计算冗余或性能不足问题，本文提出Implicit-JSCC框架。该框架将编解码建模为不动点平衡过程，通过残差收敛动态确定有效深度。结合高斯过程核演化理论，揭示了信噪比（SNR）与所需精细化深度之间的关系，实现了计算效率与重建质量的自适应平衡。
selection_source: fresh_fetch
motivation: 现有的Deep JSCC系统采用固定深度架构，无法根据信道信噪比动态调整计算量，导致在良好信道下计算浪费或在恶劣信道下性能受限。
method: 提出一种隐式平衡框架，将编解码过程转化为不动点求解，并利用高斯过程核演化理论分析信噪比对接收端迭代深度的影响。
result: 实验表明Implicit-JSCC在保持高性能的同时，支持基于残差的自适应推理，并能根据信噪比变化灵活调整计算深度。
conclusion: 该研究通过隐式深度模型和理论分析，为Deep JSCC提供了计算与质量的权衡机制，并揭示了信噪比与有效深度之间的内在联系。
---

## 摘要
深度联合信源信道编码（Deep JSCC）中一个基本的设计问题尚未得到充分探讨：在给定信道信噪比（SNR）的情况下，语义重建需要多大的有效计算深度？现有的 Deep JSCC 系统通常采用通过经验超参数调优选择的固定深度神经网络架构，这可能导致在良好信道条件下产生不必要的计算，而在严重信道噪声下细化不足。本文提出了 Implicit-JSCC，这是一种隐式平衡框架，其中语义编码和解码被建模为不动点平衡过程。有效编码器和解码器深度由基于残差的求解器收敛性决定，而非手动预定义的层数，同时平衡迭代过程中的参数共享实现了与深度无关的参数复杂度。为了分析由此产生的有效深度行为，我们开发了一个受高斯过程启发的核演化框架，将平衡迭代建模为有效深度传播过程。由于信道噪声注入在编码器和解码器之间，该分析追踪了接收端平衡迭代过程中由信道引起的表示扰动，并推导出了理论指导的深度-信噪比（depth-SNR）关系。在对系统特定参数进行离线校准后，所得模型刻画了不同信噪比下所需的接收端细化深度。大量实验表明，Implicit-JSCC 在实现具有竞争力的重建性能的同时，支持基于残差的自适应推理和可控的计算-质量权衡。该深度-信噪比模型进一步刻画了达到预定扰动容限所需的依赖于信噪比的细化深度。
