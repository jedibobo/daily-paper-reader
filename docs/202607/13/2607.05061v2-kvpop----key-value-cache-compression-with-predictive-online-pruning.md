...
motivation: 现有的 KV 缓存剔除方法多依赖静态启发式或代理分数，难以准确预测未来 Token 的效用且鲁棒性较差。
method: 提出 KVpop，通过监督学习训练评分器，利用未来注意力信号和延迟评分机制来决定 KV 缓存的保留与剔除。
...
## 摘要
键值（KV）缓存的增长是自回归解码中的主要瓶颈，因为内存和带宽随上下文长度线性扩展。现有的 KV 逐出方法通常依赖于静态启发式方法或代理评分，这些方法难以追踪未来标记（token）的效用，并在相关性发生偏移时导致脆弱的逐出决策。为解决这一问题，我们引入了 KVpop，它通过直接监督“保留或丢弃”决策来学习固定预算的 KV 逐出策略。评分器针对一种新颖的未来注意力（future-attention）目标进行训练，该目标在不实例化密集注意力图的情况下即可高效计算。我们还进一步引入了一种基于延迟记忆的评分器，这在学习型逐出方法中是独一无二的，它将评分延迟固定步数，以利用近未来上下文。在 AIME 和 HMMT 数学推理任务上，KVpop 在 Qwen3-4B 模型上实现 75% 的 KV 缓存压缩时保留了 98% 的全注意力性能，在 88% 的压缩率下保留了 97% 的性能，持续优于现有的逐出基准。Qwen3-8B 表现出更强的结果，达到了接近完整的教师模型性能。这些结果表明，利用未来注意力信号监督逐出过程可以在保持质量的同时降低内存成本。
