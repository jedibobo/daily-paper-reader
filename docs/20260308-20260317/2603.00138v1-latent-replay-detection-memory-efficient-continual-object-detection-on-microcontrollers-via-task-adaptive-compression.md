...
title_zh: 潜空间重放检测：通过任务自适应压缩在微控制器上实现内存高效的持续目标检测
...
tldr: 针对微控制器（MCU）内存极度受限导致无法进行持续学习的问题，本文提出了首个MCU持续目标检测框架LRD。该框架通过FiLM调制的任务自适应压缩技术将样本存储开销降至150字节，并结合空间多样性样本选择策略防止定位偏差。实验证明，LRD在64KB内存预算下实现了高效的持续检测，显著优于微调方法，并在多种MCU硬件上成功部署。
selection_source: carryover_cache
motivation: 现有的持续学习方法因需要存储大量原始图像而无法在内存极小的微控制器上实现目标检测任务的增量学习。
method: 提出了基于FiLM的任务自适应特征压缩技术和基于IoU空间最远点采样的空间多样性样本选择策略。
result: 在CORe50数据集上，LRD将单样本存储降至150字节，在64KB内存中可容纳400多个样本，并在多种MCU上实现4.9-97.5ms的推理延迟。
conclusion: LRD框架通过高效的潜在重放机制，首次在严格的MCU内存约束下实现了实用的持续目标检测。
---

## 摘要
在微控制器（MCU）上部署目标检测可实现智能边缘设备，但现有模型在部署后无法学习新的物体类别。现有的持续学习方法需要存储原始图像，这远超出了 MCU 仅数十 KB 的内存预算。我们提出了潜空间重放检测（LRD），这是首个在 MCU 内存约束下实现持续目标检测的框架。我们的主要贡献包括：1. 任务自适应压缩：不同于固定的 PCA，我们提出了带有 FiLM（特征线性调制）条件的学习型压缩，其中任务特定嵌入调制压缩过程，以保留每个任务分布的判别性特征；2. 空间多样性样本选择：传统采样忽略了对检测至关重要的空间信息——我们通过在 IoU 空间进行最远点采样来选择最大化边界框多样性的样本，从而防止重放中的定位偏差；3. 可部署于 MCU 的系统：我们的潜空间重放每个样本仅存储 150 字节，而图像则需 >10KB，这使得 64KB 的缓冲区能够容纳 400 多个样本。在 CORe50（50 个类别，5 个任务）上的实验表明，LRD 在初始任务上达到了 mAP@50，并在后续任务中保持了强劲性能，在严格的 MCU 约束下运行，相比朴素微调有显著提升。我们的任务自适应 FiLM 压缩和空间多样性样本选择协同工作，以保留检测能力。LRD 部署在 STM32H753ZI、ESP32-S3 和 MAX78000 MCU 上，在 64KB 内存预算内实现了每次推理 4.9-97.5ms 的延迟，首次在边缘设备上实现了实用的持续检测。
